...
from ai_personal_assistant.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from ai_personal_assistant.tauri.sidecar_logging import install_sidecar_logging

logger = logging.getLogger(__name__)


def _ensure_qwenpaw_app_not_loaded() -> None:
    if "ai_personal_assistant.app._app" in sys.modules:
        raise RuntimeError(
...
def _sync_loaded_qwenpaw_constant_cors_origins() -> None:
    constant_module = sys.modules.get("ai_personal_assistant.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from ai_personal_assistant.constant.CORS_ORIGINS at import time.
...
def _run_backend_server(log_level: str) -> None:
    ...
    from ai_personal_assistant.config.utils import write_last_api
    from ai_personal_assistant.constant import LOG_LEVEL_ENV
    from ai_personal_assistant.utils.logging import (
    ...
    if normalized_log_level in ("debug", "trace"):
        from ai_personal_assistant.cli.main import log_init_timings
    ...
    config = uvicorn.Config(
        "ai_personal_assistant.app._app:app",
        host=host,
        port=0,
        reload=False,
        workers=1,
...
def main() -> None:
    ...
    from ai_personal_assistant.constant import LOG_LEVEL_ENV, WORKING_DIR
    ...
    if not config_path.exists():
        from ai_personal_assistant.cli.init_cmd import init_cmd
