...
def install_sidecar_logging(log_path: Path) -> Path:
    ...
    logging.getLogger("ai_personal_assistant.tauri").info(
...
def _add_project_file_handler(log_path: Path) -> None:
    # Import lazily so ai_personal_assistant.constant is loaded only after desktop env setup.
    from ai_personal_assistant.utils.logging import add_project_file_handler
