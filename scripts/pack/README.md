# AIPersonalAssistant Desktop packaging scripts
...
# Output: dist/AIPersonalAssistant.app
...
# Output: dist/AIPersonalAssistant-Setup-<version>.exe
# Creates two launchers:
#   - AIPersonalAssistant.vbs (silent, no console window)
#   - AIPersonalAssistant (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/AIPersonalAssistant.app/Contents/Resources/env"
...
When users download the AIPersonalAssistant macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'AIPersonalAssistant' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the AIPersonalAssistant app -> **Open** -> in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings -> Privacy & Security**, find the message like *"AIPersonalAssistant was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/AIPersonalAssistant.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| `build_macos.sh` | One-click: build wheel → build_common → unpack into AIPersonalAssistant.app; optional zip. |
