#!/usr/bin/env bash
# One-click build: console -> conda-pack -> AIPersonalAssistant.app. Run from repo root.
...
if [ ! -t 2 ]; then
  ...
  { echo "=== $(date) AIPersonalAssistant starting ==="
...
<dict>
  ...
  <key>NSDesktopFolderUsageDescription</key><string>AIPersonalAssistant may access files in your Desktop folder if you use file-related features. You can choose Don'\''t Allow; the app will still run with limited file access.</string>
</dict>
</plist>
INFOPLIST
...
if [[ -n "${CREATE_ZIP}" ]]; then
  ZIP_NAME="${DIST}/AIPersonalAssistant-${VERSION}-macOS.zip"
