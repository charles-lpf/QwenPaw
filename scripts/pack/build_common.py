...
"""
Create a temporary conda env, install AI Personal Assistant from a wheel, run conda-pack.
...
ENV_PREFIX = "ai_personal_assistant_pack_"
...
def _pick_wheel(wheel_arg: str | None) -> Path:
    ...
    wheels = sorted(
        (REPO_ROOT / "dist").glob("ai_personal_assistant-*.whl"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Conda-pack AI Personal Assistant (temp env).",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "dist/ai_personal_assistant-*.whl."
        ),
    )
    parser.add_argument(
        "--cache-wheels",
    ...
    try:
        ...
        # Install ai_personal_assistant with all dependencies
        ...
        _run(
            [
                ...
                "pip",
                "install",
                f"ai-personal-assistant[full] @ {wheel_uri}",
            ],
            env=install_env,
        )
        ...
        # ai-personal-assistant[full]. Restore conda-managed packaging tools before packing.
        _run(
            [
                conda,
                "run",
