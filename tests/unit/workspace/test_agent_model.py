...
from ai_personal_assistant.constant import (
    LLM_BACKOFF_BASE,
    LLM_BACKOFF_CAP,
    LLM_MAX_RETRIES,
)
...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from ai_personal_assistant.config.utils import get_config_path
    from ai_personal_assistant.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr("ai_personal_assistant.config.utils.WORKING_DIR", tmp_path)
    monkeypatch.setattr("ai_personal_assistant.config.config.WORKING_DIR", tmp_path)
