...
**快速链接：** [GitHub](https://github.com/agentscope-ai/AI-Personal-Assistant) · [文档](https://ai-personal-assistant.agentscope.io/) · [许可证：Apache 2.0](LICENSE)
...
- **检查 [Open Issues](https://github.com/agentscope-ai/AI-Personal-Assistant/issues)** 以及任何 [Projects](https://github.com/agentscope-ai/AI-Personal-Assistant/projects) 或路线图标签。
...
确定满足上述条件后，可以在 `src/ai_personal_assistant/providers/provider_manager.py` 中创建新的 Provider 实例并在 `ProviderManager` 类中注册，使其成为 QwenPaw 内置的提供商。
...
- **实现：** 实现 **`BaseChannel` 的子类**（在 `src/ai_personal_assistant/app/channels/base.py` 中）：
...
- **发现：** 内置频道在 `src/ai_personal_assistant/app/channels/registry.py` 中注册。**自定义频道**从工作目录加载：放置一个模块（如 `custom_channels/telegram.py` 或包 `custom_channels/telegram/`），定义一个带有 `channel` 属性的 `BaseChannel` 子类。
- **CLI：** 用户使用以下命令安装/添加频道：
...
- **位置：** 内置 skills 位于 `src/ai_personal_assistant/agents/skills/<skill_name>/` 下。应用程序将内置和用户的 **customized_skills**（来自工作目录）合并到 **active_skills** 中；除了在目录中放置有效的 `SKILL.md` 外，不需要额外的注册。
...
- **文档：** 对 [文档](https://ai-personal-assistant.agentscope.io/)（位于 `website/public/docs/` 下）和 README 的修复和改进始终受欢迎。
...
- **讨论：** [GitHub Discussions](https://github.com/agentscope-ai/AI-Personal-Assistant/discussions)
- **Bug 和功能：** [GitHub Issues](https://github.com/agentscope-ai/AI-Personal-Assistant/issues)
